...
categories:
  - 数据库
date: 2019-04-26 22:58:00
---
最近数据库课程里需要用到分布式数据库，老师给了简单的教程，写的太烂了，我决定整理一下。
+ 配置环境：
Ubunt 16.04
MySQL
Java 1.8.0_161
Hadoop 2.6.4
Hive-common-2.3.4(Hive对MySQL驱动)
...
Hadoop需要使用ssh进行通信，首先我们需要在我们的操作系统上安装ssh。
其实我原来配置过SSH，只不过是和别的服务器连接，这里是需要和本机服务器进行连接。
首先进行安装：
```
...
```
配置ssh免密登录。进入当前用户（最好不要用root）的home目录，生成本机秘钥：
```
cd ;ssh-keygen -t rsa -P "" 
```
之后在家目录下智行`ls -al`,看看有没有隐藏目录`.ssh`，如果有就cd进去。

![upload successful](/images/pasted-31.png)

`id_rsa.pub`就是公钥了，下面我们要把咱们本机的公钥放到authorized_keys里面去，这个文件是ssh到其他服务器时候需要的公钥集合，把本机的放进去，就可以ssh本机了。最后还要改一下权限。
```
cat .ssh/id_rsa.pub >> .ssh/authorized_keys
chmod 600 .ssh/authorized_keys
```
最后输入命令 `ssh localhost`查看ssh是否配置成功
...
## 压缩下载的文件到指定文件夹 
```
tar -zxvf hadoop-2.6.3 -C /usr/local/
```
## 配置`/usr/local/hadoop/etc/hadoop/`目录下三个文件

+ core-site.xml配置如下：
`/home/tmp`处需要你mkdir一个文件夹并且设置777权限，此处替换你mkdir的新文件夹路径即可。
```
<configuration>
  <property>
...
```
+ hadoop-env.sh中进行对应的配置：
```
# The java implementation to use.
export JAVA_HOME=你的JAVA路径
export HADOOP_HOME=/usr/local/hadoop
export PATH=$PATH:/usr/local/hadoopbin
```
## 在系统环境变量中写入hadoop路径： 
...
## 启动hdfs 
使用伪分布模式，首先完成格式化 `hadoop namenode -format`，随后进到hadoop文件夹下，执行下面命令：`sbin/start-all.sh `。
最后查看是否成功`jps`。
...
vim hive-site.xml
```
在`hive-site.xml`中需要修改几个地方：
+ 这里的账户和密码替换成元数据库MySQL的账户密码，我们利用这里连接MySQL。
![upload successful](/images/pasted-38.png)
![upload successful](/images/pasted-39.png)

+ 这里需要先mkdir一个目录，然后给777权限，最后修改成你安装的位置如下;
...
## 移动JDBC驱动文件
```
cp ~/Downloads/mysql-connector-java-8.0.15.tar.gz $HIVE_HOME/lib/
```
## 初始化Hive元数据库
```
schematool -initSchema -dbType mysql
```
如果成功，这个时候我们在命令行直接输入`hive`就可以直接启动hive了，前提是你需要先打开Hadoop。

![upload successful](/images/pasted-40.png)

～马上就要五一小长假啦。这次去沈阳溜溜～O(∩_∩)O~~